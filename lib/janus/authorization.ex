defmodule Janus.Authorization do
  ...
  Authorization modules expose an API that can be used to authorize and
  ...
  Authorization modules `use Janus.Authorization` and provide a reference
  to the policy that defines authorization rules.

      defmodule MyApp.Authz do
        # All required callback definitions are injected automatically
        use Janus.Authorization, policy: MyApp.Authz.Policy
      end

  Here's how you might use those callbacks:

      iex> MyApp.Authz.authorize(some_post, :archive, moderator)
      {:ok, some_post}

      iex> MyApp.Authz.authorize(post_archived_by_admin, :unarchive, moderator)
      {:error, :not_authorized}

      iex> MyApp.Authz.scope(Post, :read, moderator)
      %Ecto.Query{}

      iex> MyApp.Authz.scope(Post, :read, moderator) |> Repo.all()
      [ ... posts the moderator can read ]

      iex> MyApp.Authz.any_authorized?(Post, :edit, moderator)
      true # there are rules allowing moderators to edit posts

      iex> MyApp.Authz.any_authorized?(Post, :delete, moderator)
      false # there are no rules that allow moderators to delete posts

  Authorization modules are a great place for additional functions that
  establish authorization patterns. Authorization modules generated by
  `mix janus.gen.authz`, for instance, contain wrappers for common
  `Ecto.Repo` functions that additionally apply authorization rules.
  """
