defmodule Janus.AuthorizationTest do
  ...
  describe "validate_authorized/4" do
    setup do
      user = user_fixture()
      ...
      policy =
        %Janus.Policy{}
        |> allow(:edit, Thread, where: [creator_id: user.id, archived: false])

      %{thread: thread, policy: policy}
    end

    test "shouldn't add error if change is authorized", %{thread: thread, policy: policy} do
      allowed =
        thread
        |> Thread.changeset(%{title: "new title"})
        |> Auth.validate_authorized(:edit, policy)

      assert %Ecto.Changeset{valid?: true} = allowed
    end

    test "should add error if action not allowed by policy", %{thread: thread, policy: policy} do
      denied =
        thread
        |> Thread.changeset(%{title: "new title"})
        |> Auth.validate_authorized(:super_edit, policy)

      assert %Ecto.Changeset{valid?: false} = denied

      assert [current_actor: {"is not authorized to change this resource", [policy: ^policy]}] =
               denied.errors
    end

    test "should add error if resource is not authorized prior to change", %{
      thread: thread,
      policy: policy
    } do
      denied =
    ...
    test "should add error if resource is not authorized after change", %{
      thread: thread,
      policy: policy
    } do
      denied =
        thread
        |> Thread.changeset(%{title: "new title", archived: true})
        |> Auth.validate_authorized(:edit, policy)

      assert %Ecto.Changeset{valid?: false} = denied

      assert [current_actor: {"is not authorized to make these changes", [policy: ^policy]}] =
               denied.errors
    end

    test "should accept custom :error_key", %{thread: thread, policy: policy} do
      denied =
        thread
        |> Thread.changeset(%{archived: true})
        |> Auth.validate_authorized(:edit, policy, error_key: :custom_key)

      assert [custom_key: {_, _}] = denied.errors
    end

    test "should accept custom :pre_message", %{thread: thread, policy: policy} do
      denied =
        %{thread | archived: true}
        |> Thread.changeset(%{title: "new title"})
        |> Auth.validate_authorized(:edit, policy, pre_message: "custom message")

      assert [current_actor: {"custom message", _}] = denied.errors
    end

    test "should accept custom :post_message", %{thread: thread, policy: policy} do
      denied =
        thread
        |> Thread.changeset(%{archived: true})
        |> Auth.validate_authorized(:edit, policy, post_message: "custom message")
